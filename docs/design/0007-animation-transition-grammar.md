...
| AC-2 | drag가 시작된 뒤 release되면 위치와 무관하게 `Dragging → Falling → Landing → Recovering → IdleHub`를 순서대로 거치며 중간 상태를 건너뛰지 않는다. | fake clock/surface runtime 테스트에서 상태·clip trace를 exact sequence로 비교한다. |
...
stateDiagram-v2
  ...
  Landing --> Recovering: impact complete
...
| Landing/Recovering | pose=GroundedCompressed → Standing | impact와 회복 clip을 끝낸 뒤에만 IdleHub. |
...
| drag release 직후 실제 낙하 거리가 0px일 수 있다. | Falling을 최소 한 Tick 유지한 뒤 같은 surface에 landing해도 fall→land 인과 trace와 최소 anticipation frame을 보장한다. |
