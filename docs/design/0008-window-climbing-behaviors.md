...
| ID | 수용 기준 | 검증 방법 |
|---|---|---|
| AC-1 | 애니메이션 패키지는 `rope_climb_*`, `free_climb_*` 의미 ID를 제공한다. | asset 파이프라인 테스트와 별도 아트 작업에서 검증한다. |
...
| AC-4 | 새 클립의 raster artwork과 pack 등록은 아트 작업 단위에서 제공한다. | asset 파이프라인 테스트. |
...
| 상태 | 저장/이벤트 | UI 반응 |
|---|---|---|
| RopeClimbPreparing / FreeClimbPreparing | climb plan과 anchor를 보존한다. | prepare clip을 재생한다. |
| RopeClimbing / FreeClimbing | fixed X, 시작 Y, 목표 Y를 보존하고 Tick마다 후보 top을 조회한다. | loop clip 재생 및 위로 이동한다. |
| ClimbFinishing | 도착한 surface를 support로 보존한다. | finish clip 후 idle한다. |
...
| 새 art pack이 런타임보다 늦게 등록될 수 있다. | runtime은 semantic clip만 선언하고 asset 작업이 pack coverage를 검증한다. |

## 11. 착수 순서

- [ ] 0. art producer가 semantic clip raster와 pack coverage를 추가한다. (AC-1, AC-4)
